...
export default function FooterSection({ contact }: FooterSectionProps) {
  ...
  return (
    <footer className="relative bg-[var(--color-primary)] px-6 py-12 text-center text-white overflow-hidden">
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 invert" style={geometricBackground(0.08)} />

      <div className="relative z-10">
        <Reveal variant="up">
          <p className="text-lg font-semibold tracking-wide">Hubungi Kami</p>
        </Reveal>
            ...
            {instagramUrl && (
              <Reveal variant="up" delay={120}>
                <a
                  ...
                  className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-[var(--color-primary)] transition hover:opacity-90 hover:scale-110"
                >
        ...
        {contactHandle && (
          <Reveal variant="up" delay={360}>
            <p className="mt-6 text-sm font-medium opacity-90">{contactHandle}</p>
          </Reveal>
        )}
      </div>
    </footer>
