...
import { SURFACE } from '../components/ornaments';
...
export default function FooterSection({ contact }: FooterSectionProps) {
  return (
    <footer
      className="border-t border-[#D4AF37]/30 px-6 py-12 text-center text-neutral-200"
      style={{ backgroundColor: SURFACE }}
    >
      <Reveal variant="up">
        <p className="text-sm font-semibold uppercase tracking-[0.35em] text-[#D4AF37]">Hubungi Kami</p>
      </Reveal>
          ...
          {contact.contact_instagram_url && (
            <Reveal variant="up" delay={120}>
              <a
                ...
                className="flex h-11 w-11 items-center justify-center rounded-full border border-[#D4AF37]/60 text-[#D4AF37] transition hover:bg-[#D4AF37]/10 hover:scale-110"
              >
      ...
      {contact.contact_handle && (
        <Reveal variant="up" delay={360}>
          <p className="mt-6 text-sm font-medium text-neutral-400">{contact.contact_handle}</p>
        </Reveal>
      )}
    </footer>
  );
