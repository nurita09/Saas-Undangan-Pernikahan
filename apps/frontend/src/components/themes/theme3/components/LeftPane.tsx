...
export default function LeftPane({
  ...
  return (
    <div className="hidden lg:flex lg:flex-1 lg:sticky lg:top-0 lg:h-screen relative flex-col items-center justify-end pb-24 overflow-hidden shadow-[inset_-20px_0_40px_rgba(0,0,0,0.5)] z-10">
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-t from-[var(--color-secondary)]/95 via-[var(--dk-wine)]/25 to-black/15" />
      <div className="relative z-20 text-center text-white max-w-2xl px-8">
        <DecoFan className="mx-auto h-16 w-auto" />
        <p className="mt-5 text-sm tracking-[0.5em] font-medium uppercase text-[var(--color-primary)]">
          The Wedding Of
        </p>
        <h1 className="mt-3 font-script text-7xl leading-tight text-white drop-shadow-xl xl:text-8xl">
          {couple.groom_name}{" "}
          <span className="text-[var(--color-primary)]">&amp;</span>{" "}
          {couple.bride_name}
        </h1>
        {weddingDate && (
          <p className="mt-6 font-serif text-2xl tracking-[0.35em] text-[var(--color-primary)] drop-shadow-md">
